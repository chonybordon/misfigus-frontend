...
def login_user(email):
    ...
    # Get OTP from backend logs (we need to read it)
    time.sleep(1)
    ...
    try:
        with open('/var/log/supervisor/backend.err.log', 'r') as f:
            ...
            for line in reversed(lines[-20:]):
                if 'OTP:' in line and email.split('@')[0] in ''.join(lines[-30:]):
...
def setup_user_profile(token, user_id):
    ...
    # Accept terms
    response = requests.post(f"{BASE_URL}/api/user/accept-terms", headers=headers)
    
    # Set location
    response = requests.put(f"{BASE_URL}/api/user/profile", headers=headers, json={
...
def create_album(token, name="Test Album"):
    """Create an album for the user"""
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    response = requests.post(f"{BASE_URL}/api/albums", headers=headers, json={
        "name": name,
        "year": 2024
    })
...
def test_chat_limit_error_structure():
    ...
    # Create albums and stickers
    print("\nCreating albums and stickers...")
    album_a = create_album(token_a, "Album A")
    album_b = create_album(token_b, "Album B")
    album_c = create_album(token_c, "Album C")
    
    if not all([album_a, album_b, album_c]):
        print("FAILED: Could not create albums")
        return False
    
    # Add stickers
    add_sticker_to_album(token_a, album_a, "1", "have")
    add_sticker_to_album(token_a, album_a, "2", "need")
    add_sticker_to_album(token_b, album_b, "1", "need")
    add_sticker_to_album(token_b, album_b, "2", "have")
    add_sticker_to_album(token_c, album_c, "3", "have")
    add_sticker_to_album(token_c, album_c, "4", "need")
    ...
    exchange_ab = create_exchange(token_a, user_b['id'], 
                                   [{"album_id": album_a, "sticker_number": "1"}],
                                   [{"album_id": album_b, "sticker_number": "2"}])
    if not exchange_ab:
    ...
    exchange_ca = create_exchange(token_c, user_a['id'],
                                   [{"album_id": album_c, "sticker_number": "3"}],
                                   [{"album_id": album_a, "sticker_number": "1"}])
    if not exchange_ca:
